...
bool NodePath::Load(size_t Size,char* Data)
{
    //printf("BEGIN NODE PATH (S: %zu)\n",Size);
    int dPos = 0;
    
    //Get path name
    size_t NameLen = (unsigned char)Data[dPos++];
    string Name(&Data[dPos],NameLen); dPos += NameLen + 1;
    //cout << "Name: " << Name << "." << endl;
...
bool NodePath::LoadBlock(const string& BlockID,size_t Size,char* Data)
{
    return true;
}